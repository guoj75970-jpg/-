...
BACKUP=XLSX.with_name('7月人审量_追加7.24-7.26前备份.xlsx')
HTML_BACKUP=ROOT/'index_追加7.24-7.26前备份.html'
...
# Rebuild dashboard from all source CSVs through 7.26.
...
sub(r'(<div>数据周期: 2026-07-01 至 )2026-07-23',r'\g<1>2026-07-26')
text=text.replace('7月累计23天','7月累计26天').replace('7月24小时时段数据（7.1-7.23累计）','7月24小时时段数据（7.1-7.26累计）').replace('7月人员排行(7.1-7.23累计)','7月人员排行(7.1-7.26累计)')
HTML.write_text(text,encoding='utf-8')
dash={'through':'7.26','cum_actual':cum_actual,'cum_trans':cum_trans,'people_total':int(people_total),'eff':eff,'cum_viol':cum_viol,'cum_queue':cum_queue,'daily':{d:{'actual':a,'trans':t,'queue':q,'viol':v} for d,a,t,q,v in zip(labels,audit,trans,queues,viol)},'person_rank':pr,'apps':dict(apps),'hourly':hr,'zt_volumes':zv,'zt_queues':zq}
(ROOT/'dashboard_7.26_summary.json').write_text(json.dumps(dash,ensure_ascii=False,indent=2),encoding='utf-8')
